...
class RagFormer:
    ...
    def _retriever(self, query: str, k: int, verbose: bool) -> List[Document]:
        ...
        relevant_docs = [
            Document(page_content=self.docs[idx], id=str(idx)) for idx in args[:k]]
        # Display results
        if verbose:
    ...
    def as_retriever(self, search_kwargs: dict = {'k': 4}, verbose=False) -> Runnable:
        if not isinstance(search_kwargs, dict):
            raise ValueError(f"search_kwargs must be a dict. Got {
                             type(search_kwargs)}")

        if 'k' not in search_kwargs:
            search_kwargs['k'] = 4

        _retriever = functools.partial(
            self._retriever, k=search_kwargs['k'], verbose=verbose)
